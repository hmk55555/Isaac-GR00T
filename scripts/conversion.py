...
# Try to import from lerobot, but define defaults if not available
try:
    ...
    # Define constants if they don't exist in the current lerobot version
    ...
    LEGACY_EPISODES_STATS_PATH = "meta/episodes_stats.json"
    LEGACY_TASKS_PATH = "meta/tasks.jsonl"
    
    def serialize_dict(d):
        """Serialize dict to JSON-compatible format."""
        import json
        return json.loads(json.dumps(d, default=str))
    
    def unflatten_dict(d, sep="."):
        """Unflatten a dict with dotted keys."""
        result = {}
...
# v3.0 format (source)
V30_DATA_PATH_TEMPLATE = "data/chunk-{chunk_index:03d}/file-{file_index:03d}.parquet"
...
def convert_data(root: Path, new_root: Path, episode_records: list[dict[str, Any]]) -> None:
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files"):
        source_path = root / V30_DATA_PATH_TEMPLATE.format(chunk_index=chunk_idx, file_index=file_idx)
...
def convert_videos(root: Path, new_root: Path, episode_records: list[dict[str, Any]], video_keys: list[str]) -> None:
    ...
    for video_key in video_keys:
        ...
        for (chunk_idx, file_idx), records in tqdm.tqdm(
            ...
            src_path = root / V30_VIDEO_PATH_TEMPLATE.format(
                video_key=video_key,
